...
export class BingVideosEngine implements SearchEngine {
  ...
  async executeSearch(query: string, page: number = 1, timeFilter?: TimeFilter, context?: EngineContext): Promise<SearchResult[]> {
    ...
    if (timeFilter && timeFilter !== "any") {
      ...
      if (freshMap[timeFilter]) url += `&qft=+filterui:videoage-lt${freshMap[timeFilter].toLowerCase()}`;
    }
    ...
    $(".mc_vtvc, .dg_u").each((_, el) => {
      const meta = $(el).attr("data-video") || $(el).find("[data-video]").attr("data-video") || "";
      try {
    ...
    if (results.length === 0) {
      $(".mc_vtvc_meta, .vrhdata").each((_, el) => {
        const title = $(el).find(".mc_vtvc_title, .mc_vtvc_meta_row").text().trim();
