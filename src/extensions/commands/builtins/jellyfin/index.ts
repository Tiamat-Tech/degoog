  ...
  async execute(
    ...
    try {
      ...
      const [hintsRes, peopleRes] = await Promise.all([
        fetch(
          `${jellyfinUrl}/Search/Hints?searchTerm=${encodeURIComponent(term)}&Limit=${perPage}&StartIndex=${startIndex}&IncludeItemTypes=Movie,Series,Episode,Audio,MusicAlbum,MusicArtist`,
          { headers: authHeaders },
        ),
        fetch(
      ...
      if (personIds.length > 0) {
        ...
        const personItemsData = (await personItemsRes.json()) as {
          Items?: Record<string, unknown>[];
        };
      ...
      for (const item of personItems) {
        ...
        if (id && !seen.has(id)) {
          seen.add(id);
          const itemPeople =
            (item["People"] as Record<string, unknown>[] | undefined) || [];
      ...
      const results = allItems
        .map((item) => {
          ...
          const imageTags = item["ImageTags"] as
