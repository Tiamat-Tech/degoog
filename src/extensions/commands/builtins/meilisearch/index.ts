...
export const meilisearchCommand: BangCommand = {
  ...
  async execute(
    ...
    const indexes = asString(stored["indexes"])
      .split(",")
      .map((s: string) => s.trim())
      .filter(Boolean);
      ...
      const results = allHits
        .map(({ hit, index }) => {
          ...
          const content = String(
            hit[contentField] || hit["metadata_summary"] || "",
          );
          ...
          const tags = [
            ...
            type
              ? `<span class="result-engine-tag">${escHtml(type)}</span>`
              : "",
            source
              ? `<span class="result-engine-tag">${escHtml(source)}</span>`
              : "",
          ]
            .filter(Boolean)
            .join("");
